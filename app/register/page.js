...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    ...
    confirm_password: '', role: 'student', faculty: ''
  })
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          password: form.password, role: form.role, faculty: form.faculty,
        }),
      })
  ...
  const faculties = [
    'Business Administration', 'Engineering', 'Digital Innovation',
    'Medicine', 'Liberal Arts', 'Communication Arts',
    'Science & Technology', 'Law', 'Architecture', 'Education', 'Computer Science'
  ]
        ...
        <div style={{ width: '100%', maxWidth: 520 }} className="card">
          ...
          <div style={{
            ...
            <div style={{
              ...
              background: 'linear-gradient(135deg, rgba(244,7,86,0.1) 0%, rgba(100,7,244,0.05) 100%)',
            }}>
              <div style={{
                display: 'inline-flex', alignItems: 'center', gap: 6,
                background: 'rgba(244,7,86,0.15)', border: '1px solid rgba(244,7,86,0.3)',
                color: '#F40756', fontSize: 10, fontWeight: 800,
                padding: '4px 14px', borderRadius: 20, letterSpacing: 1.5, marginBottom: 16
              }}>UNIVERSITY NEWS PLATFORM</div>
            ...
            <div style={{ padding: '2rem' }}>
              {error && (
                <div style={{
                  background: 'rgba(244,7,86,0.1)', border: '1px solid rgba(244,7,86,0.3)',
                  borderRadius: 10, padding: '12px 16px',
                  color: '#ff6b9d', fontSize: 13, marginBottom: '1.25rem',
                  display: 'flex', alignItems: 'center', gap: 8
                }}>⚠️ {error}</div>
              )}
              {success && (
                <div style={{
                  background: 'rgba(0,196,122,0.1)', border: '1px solid rgba(0,196,122,0.3)',
                  borderRadius: 10, padding: '12px 16px',
                  color: '#00c47a', fontSize: 13, marginBottom: '1.25rem',
                  display: 'flex', alignItems: 'center', gap: 8
                }}>✅ {success}</div>
              )}
